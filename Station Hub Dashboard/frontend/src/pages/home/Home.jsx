...
export default function HomeLayout() {
  ...
  const navbar   = dark ? "bg-[#1a1d27] border-[#2a2d3e]"  : "bg-white border-gray-200";
  const sidebar  = dark ? "bg-[#1a1d27] border-[#2a2d3e]"  : "bg-white border-gray-200";
  ...
  const bellBg   = dark ? "bg-[#2a2d3e] text-gray-300 hover:bg-[#353849] hover:text-orange-400" : "bg-gray-100 text-gray-500 hover:text-orange-500 hover:bg-orange-50";
  const userCard = dark ? "bg-orange-500/10 border-orange-500/30" : "bg-orange-500/10 border-orange-500/30";
  const mainBg   = dark ? "bg-[#0f1117]"                   : "bg-gray-50";

  return (
    <div className={`flex flex-col h-screen font-sans transition-colors duration-300 ${bg}`}>

      {/* ── TOP NAVBAR ── */}
      <header className={`h-16 w-screen border-b flex items-center justify-between pr-6 z-10 transition-colors duration-300 ${navbar}`}>

        <div className={`flex items-center w-52 border-b px-2 py-0 justify-between gap-4 ${dark ? "border-[#2a2d3e]" : "border-gray-100"}`}>
          {/* Hamburger */}
        ...
        <div className="flex items-center gap-3">
          <div className={`w-9 h-9 rounded-full flex items-center justify-center transition-colors cursor-pointer ${bellBg}`}>
            <IconBell />
          </div>
      ...
      <div className="flex h-[calc(100vh-64px)]">
        ...
        {/* ── SIDEBAR ── */}
        <aside className={`flex-shrink-0 h-full border-r flex flex-col transition-all duration-300 overflow-hidden ${sidebar} ${sidebarOpen ? "w-52" : "w-14"}`}>
          ...
          {/* Bottom user card */}
          <div className={`border-t px-2 py-4 ${dark ? "border-[#2a2d3e]" : "border-gray-100"}`}>
            <div className={`flex items-center gap-2 px-2 py-2 rounded-lg border ${userCard} ${!sidebarOpen ? "justify-center" : ""}`}>
              ...
              {sidebarOpen && (
                <div className="min-w-0 flex items-center justify-between gap-2 flex-1">
                  <div className="min-w-0">
                    <p className={`text-xs font-medium truncate ${txt}`}>Sadeed Javaid</p>
                    <p className={`text-xs truncate ${sub}`}>sadeed@example.com</p>
                  </div>
                  <button
        ...
        {/* ── MAIN CONTENT ── */}
        <main className={`flex-1 overflow-auto transition-colors duration-300 ${mainBg}`}>
          <Outlet />
        </main>
